...
erDiagram
    ...
    PROJECT {
        ...
        string logDir "directory/.webdev/"
    }
    ...
    SERVER {
        ...
        string logDir "absolute path to .webdev/"
    }
...
graph LR
    ...
    subgraph "Gateway :3333"
        ...
        CLIENT_JS["/__webdev.js<br/>GET (HTTP)"]
    end
...
sequenceDiagram
    ...
    Note over Adapter: Adapter injects serverId into pages<br/>as window.__WEBDEV_SERVER__
...
sequenceDiagram
    ...
    Client->>Client: serverId = window.__WEBDEV_SERVER__<br/>(stable: projectId:type)
...
1. Extension detects dev pages via `<meta name="webdev">` tag
...
1. **Relative script loading** — Vite adapter loads `/__webdev.js` from vite's own origin (relative URL), not from the absolute gateway URL. Vite serves it via middleware.
